$.fn.upload = function(opts) {
	var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div></div>';
	var defaults = {
...
var Now;
	...
	this.each(function() {
		var _this = $(this);
		//先添加上file按钮和上传列表
		var instanceNumber = $('.uploadify-button').length + 1;
					...
					xhr.onreadystatechange = function(e) {
						if (xhr.readyState == 4) {
							if (xhr.status == 200) {
								...
								option.onUploadSuccess && option.onUploadSuccess(xhr.responseText,Now);
								//在指定的间隔时间后删掉进度条
								//Now.css("background-image","url("+ xhr.responseText +")");
								_this.find('#fileupload_' + instanceNumber + '_' + file.index).fadeOut(500);
								setTimeout(function() {
							...
							option.onUploadComplete && option.onUploadComplete(xhr.responseText);
							//清除文件选择框中的已有值
